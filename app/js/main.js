...
$(function () {
  ...
  var setTheme = function () {
    if (localStorage.dark === '') {
      ...
      $('.floated-colors-btn .os-toggler-w').removeClass('on');
    } else {
      ...
      $('.floated-colors-btn .os-toggler-w').addClass('on');
    }
  }
  ...
  $('.search-suggest-input').on('keydown', function (e) {
    ...
    this.TTL = setTimeout(()=>{
      ...
      if(value.length < 3){ // close and remove all previous resluts
        return $('.search-with-suggestions-content').html("");
        // return $('.search-with-suggestions-w').fadeOut();
      }
      app.trigger("search",value,(id, icon, title, type="button")=>{
        if(!id) return;
        let items = {
          "button" :  ["ssg-items ssg-items-boxed",(image, title, link)=>`
            <a class="ssg-item" href="${link}">
              ${ image ? `<div class="item-media" style="background-image: url('${image}')"></div>` : "" }
              <div class="item-name ${ image ? '' : 'text-center'}">${title}</div>
            </a>
          `],
          "list"  :  ["ssg-items ssg-items-list",(image, title, link)=>`
            <a class="ssg-item" href="${link}">
              ${ image ? `<div class="item-media" style="background-image: url('${image}')"></div>` : "" }
              <div class="item-name ${ image ? '' : 'text-center'}">${title}</div>
            </a>
          `],
          "file" : ["ssg-items ssg-items-blocks",(icon, title, link)=>`
            <a class="ssg-item" href="${link}">
              <div class="item-icon">
        ...
        if(!(type in items))
          type = button;
        let searchGroup = `
          <div class="search-suggestions-group" id="${id}">
            <div class="ssg-header">
              <div class="ssg-icon">
                <div class="${icon}"></div>
              </div>
              <div class="ssg-name">${title}</div>
            </div>
            <div class="ssg-content"><div class="${items[type][0]}">${LOADER_HTML}</div></div>
          </div>`;
        let searchGroupEl = $(`.search-with-suggestions-content #${id}`);
        if(searchGroupEl.length == 0){
          $('.search-with-suggestions-content').append(searchGroup);
          searchGroupEl = $(`.search-with-suggestions-content #${id}`);
          searchGroupEl.get(0).addItemFn = items[type][1];
          searchGroupEl.get(0).itemType = items[type][0];
        }else{
          $(`.search-with-suggestions-content #${id} .ssg-content .ssg-items`).html(LOADER_HTML);
        }
        $(`.search-with-suggestions-content #${id} .ssg-content .ssg-items > center`).css("width","100%");
        let uuid = generateUUID("xxxxxxxxxxxxxxx");
        searchGroupEl.attr("rel",uuid);  /// uuid for search scope
        type = searchGroupEl.get(0).addItemFn;
        return {
          noResults(){
            if($(`.search-with-suggestions-content #${id}[rel=${uuid}]`).length == 0)
              return; // prevent add old result ignore result
            $(`.search-with-suggestions-content #${id} .ssg-content .ssg-items`).html(`
              <div class="ssg-nothing-found" style="
                  display: block;
                  width: 100%;
              ">
                  <div class="icon-w">
          ...
          addItem(icon, title, link) {
            console.debug(`addItem ${uuid}`,$(`.search-with-suggestions-content #${id}[rel=${uuid}]`),type);
            if($(`.search-with-suggestions-content #${id}[rel=${uuid}]`).length == 0)
               return; // prevent add old result ignore result
            let content = $(`.search-with-suggestions-content #${id} .ssg-content .ssg-items`);
            if($(`.search-with-suggestions-content #${id} .ssg-content .ssg-items .ssg-item`).length == 0)
              $(`.search-with-suggestions-content #${id} .ssg-content  .ssg-items`).html("");
            $(`.search-with-suggestions-content #${id} .ssg-content .ssg-items`).append(type(icon, title, link));
          }
        }
      })
  ...
  // #24. Windows controls
  $('.top-menu-controls [win], #loader-wrapper .top-icon [win]').click(function (argument) {
    const { remote } = nodeRequire('electron');
    var options = $(this).attr("win").split("|"),win = remote.BrowserWindow.getFocusedWindow();
    var rel = $(this).attr("rel");
    if(rel && rel in global)
        win = global[rel];
    if(options.length == 1)
      win[options[0]]();
    else
      win[win[options[0]]() ? options[1] : options[2]]();
